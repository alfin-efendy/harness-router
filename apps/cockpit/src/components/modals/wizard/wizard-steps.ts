...
// permissions gates on component-backed or untrusted-source risk;
// settings gates on the manifest declaring [[settings]]; connect gates on
// either a top-level auth requirement, providers always needing a
// connection, or a component's declared OAuth profile.
export function planWizardSteps(i: WizardPlanInput): WizardStepId[] {
  const steps: WizardStepId[] = ["overview"];
  if (i.componentBacked || i.trustRequired) steps.push("permissions");
  steps.push("install");
