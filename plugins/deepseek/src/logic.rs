//! The `deepseek` provider's OpenAI-format configuration.
//!
...
//! provider's own configuration, transcribed from the `deepseek`
...
/// The `deepseek` descriptor, as wire configuration.
///
/// Descriptor facts that drive it: `base_url = Some("https://api.deepseek.com/v1")`,
...
///
/// The context-window table is EMPTY on purpose. DeepSeek's `/models` response
...
mod tests {
    ...
    #[test]
    fn config_matches_the_deepseek_descriptor() {
        assert_eq!(CONFIG.resolve_base_url(None), "https://api.deepseek.com/v1");
        assert_eq!(
            CONFIG.chat_url("https://api.deepseek.com/v1"),
            "https://api.deepseek.com/v1/chat/completions"
        );
        assert_eq!(
            CONFIG.models_url("https://api.deepseek.com/v1"),
            "https://api.deepseek.com/v1/models"
        );
        assert_eq!(CONFIG.provider_label, "DeepSeek");
    }

    #[test]
    fn chat_body_uses_the_legacy_max_tokens_field() {
        // The `deepseek` descriptor leaves `uses_max_completion_tokens` false, so
