//! First-party Groq provider component.
//!
//! Exports `ryuzi:provider/provider@0.2.0` (`list-models` + `complete`) over
//! Groq's OpenAI-compatible `/models` and `/chat/completions` endpoints.
//!
//! # The component never sees the user's API key
//! Groq is an API-KEY provider: the credential belongs to the user, not the
...
//! per the `groq` descriptor's `AuthScheme::Bearer`, enforces the manifest
...
//! [`logic::CONFIG`], the `groq` descriptor transcribed into that crate's
