//! The `groq` provider's OpenAI-format configuration.
//!
...
//! provider's own configuration, transcribed from the `groq`
...
/// The `groq` descriptor, as wire configuration.
///
/// Descriptor facts that drive it: `base_url = Some("https://api.groq.com/openai/v1")`,
...
///
/// The context-window table is EMPTY on purpose. Groq's `/models` response
...
mod tests {
    ...
    #[test]
    fn config_matches_the_groq_descriptor() {
        assert_eq!(
            CONFIG.resolve_base_url(None),
            "https://api.groq.com/openai/v1"
        );
        assert_eq!(
            CONFIG.chat_url("https://api.groq.com/openai/v1"),
            "https://api.groq.com/openai/v1/chat/completions"
        );
        assert_eq!(
            CONFIG.models_url("https://api.groq.com/openai/v1"),
    ...
    fn chat_body_uses_the_legacy_max_tokens_field() {
        // The `groq` descriptor leaves `uses_max_completion_tokens` false, so
