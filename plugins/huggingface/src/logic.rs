//! The `huggingface` provider's OpenAI-format configuration.
//!
...
//! provider's own configuration, transcribed from the `huggingface`
...
/// The `huggingface` descriptor, as wire configuration.
///
/// Descriptor facts that drive it: `base_url = Some("https://router.huggingface.co/v1")`,
...
///
/// The context-window table is EMPTY on purpose. Hugging Face's `/models` response
...
mod tests {
    ...
    #[test]
    fn config_matches_the_huggingface_descriptor() {
        assert_eq!(
            CONFIG.resolve_base_url(None),
            "https://router.huggingface.co/v1"
        );
        assert_eq!(
            CONFIG.chat_url("https://router.huggingface.co/v1"),
            "https://router.huggingface.co/v1/chat/completions"
        );
        assert_eq!(
            CONFIG.models_url("https://router.huggingface.co/v1"),
            "https://router.huggingface.co/v1/models"
        );
        assert_eq!(CONFIG.provider_label, "Hugging Face");
    }

    #[test]
    fn chat_body_uses_the_legacy_max_tokens_field() {
        // The `huggingface` descriptor leaves `uses_max_completion_tokens` false, so
