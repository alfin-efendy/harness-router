...
//! Mistral is an API-KEY provider: the credential belongs to the user, not the
...
//! per the `mistral` descriptor's `AuthScheme::Bearer`, enforces the manifest
...
//! [`logic::CONFIG`], the `mistral` descriptor transcribed into that crate's
