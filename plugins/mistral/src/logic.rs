//! The `mistral` provider's OpenAI-format configuration.
//!
...
//! provider's own configuration, transcribed from the `mistral`
...
/// The `mistral` descriptor, as wire configuration.
///
/// Descriptor facts that drive it: `base_url = Some("https://api.mistral.ai/v1")`,
...
///
/// The context-window table is EMPTY on purpose. Mistral's `/models` response
...
mod tests {
    ...
    #[test]
    fn config_matches_the_mistral_descriptor() {
        assert_eq!(CONFIG.resolve_base_url(None), "https://api.mistral.ai/v1");
        assert_eq!(
            CONFIG.chat_url("https://api.mistral.ai/v1"),
            "https://api.mistral.ai/v1/chat/completions"
        );
        assert_eq!(
            CONFIG.models_url("https://api.mistral.ai/v1"),
            "https://api.mistral.ai/v1/models"
        );
        assert_eq!(CONFIG.provider_label, "Mistral");
    }

    #[test]
    fn chat_body_uses_the_legacy_max_tokens_field() {
        // The `mistral` descriptor leaves `uses_max_completion_tokens` false, so
