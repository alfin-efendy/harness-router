//! First-party xAI provider component.
//!
//! Exports `ryuzi:provider/provider@0.2.0` (`list-models` + `complete`) over
//! xAI's OpenAI-compatible `/models` and `/chat/completions` endpoints.
//!
//! # The component never sees the user's API key
//! xAI is an API-KEY provider: the credential belongs to the user, not the
...
//! per the `xai` descriptor's `AuthScheme::Bearer`, enforces the manifest
...
//! [`logic::CONFIG`], the `xai` descriptor transcribed into that crate's
