//! First-party OpenRouter provider component.
//!
//! Exports `ryuzi:provider/provider@0.2.0` (`list-models` + `complete`) over
//! OpenRouter's OpenAI-compatible `/models` and `/chat/completions` endpoints.
//!
//! # The component never sees the user's API key
//! OpenRouter is an API-KEY provider: the credential belongs to the user, not the
...
//! per the `openrouter` descriptor's `AuthScheme::Bearer`, enforces the manifest
...
//! [`logic::CONFIG`], the `openrouter` descriptor transcribed into that crate's
