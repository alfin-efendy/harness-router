//! The `openrouter` provider's OpenAI-format configuration.
//!
...
//! provider's own configuration, transcribed from the `openrouter`
...
/// The `openrouter` descriptor, as wire configuration.
///
/// Descriptor facts that drive it: `base_url = Some("https://openrouter.ai/api/v1")`,
...
///
/// The context-window table is EMPTY on purpose. OpenRouter's `/models` response
...
mod tests {
    ...
    #[test]
    fn config_matches_the_openrouter_descriptor() {
        assert_eq!(
            CONFIG.resolve_base_url(None),
            "https://openrouter.ai/api/v1"
        );
        assert_eq!(
            CONFIG.chat_url("https://openrouter.ai/api/v1"),
            "https://openrouter.ai/api/v1/chat/completions"
        );
        assert_eq!(
            CONFIG.models_url("https://openrouter.ai/api/v1"),
            "https://openrouter.ai/api/v1/models"
        );
        assert_eq!(CONFIG.provider_label, "OpenRouter");
    }

    #[test]
    fn chat_body_uses_the_legacy_max_tokens_field() {
        // The `openrouter` descriptor leaves `uses_max_completion_tokens` false, so
