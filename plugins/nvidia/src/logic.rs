//! The `nvidia` provider's OpenAI-format configuration.
//!
...
//! provider's own configuration, transcribed from the `nvidia`
...
/// The `nvidia` descriptor, as wire configuration.
///
/// Descriptor facts that drive it: `base_url = Some("https://integrate.api.nvidia.com/v1")`,
...
///
/// The context-window table is EMPTY on purpose. NVIDIA NIM's `/models` response
...
mod tests {
    ...
    #[test]
    fn config_matches_the_nvidia_descriptor() {
        assert_eq!(
            CONFIG.resolve_base_url(None),
            "https://integrate.api.nvidia.com/v1"
        );
        assert_eq!(
            CONFIG.chat_url("https://integrate.api.nvidia.com/v1"),
            "https://integrate.api.nvidia.com/v1/chat/completions"
        );
        assert_eq!(
            CONFIG.models_url("https://integrate.api.nvidia.com/v1"),
            "https://integrate.api.nvidia.com/v1/models"
        );
        assert_eq!(CONFIG.provider_label, "NVIDIA NIM");
    }

    #[test]
    fn chat_body_uses_the_legacy_max_tokens_field() {
        // The `nvidia` descriptor leaves `uses_max_completion_tokens` false, so
