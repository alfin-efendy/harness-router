...
impl Store {
    ...
    /// see the `plugin_oauth_profile_tokens` table for why this is a separate
    /// table rather than a schema change to the existing one.
            ...
            .with_conn(|c| {
                ...
                for table in table_names {
                    ...
                    let col_list = columns
                        ...
                        .collect::<Vec<_>>()
                        .join(", ");
                    let order_list = (1..=columns.len())
                        .map(|i| i.to_string())
                        .collect::<Vec<_>>()
                        .join(", ");
                    let select_sql =
                        format!("SELECT {col_list} FROM \"{table}\" ORDER BY {order_list}");
